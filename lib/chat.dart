...
class _ChatState extends State<Chat> {
  ...
  Future<void> getChatbotReply(String userReply) async {
    ...
    var response = await http.get(Uri.parse(
        "http://api.brainshop.ai/get?bid=167640&key=CHi7zETXqF9tNXT6&uid=anmolk03&msg=userReply"));
    var data = jsonDecode(response.body);
    print("Chatbot: " + data["cnt"]);
    // print(data["cnt"]);
...
class Message extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: index % 2 == 0 ? chatbotMessage(context) : myMessage(context),
      ),
    );
